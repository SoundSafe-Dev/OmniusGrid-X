...
**Assessment.** 27 blocking CI jobs gate every change and the count is itself asserted so it cannot go stale. CORRECTED 2026-08-20 (FS-797): this said **23**, a figure stale by an unknown margin, while the README said **31** — and BOTH were wrong. The guard that was supposed to stop the number going stale collected job names with a regex over two-space-indented keys, which in a GitHub workflow also matches the `on:` triggers; `pull_request:` and `push:` were counted as jobs in each of the two workflows, inflating the total by four. The README had been updated to match the parser's own error, and this catalogue — from which the SSP is generated — carried a third number. The guard now reads the parsed `jobs` mapping, and asserts that no trigger is counted as a job. **27 blocking, 1 advisory.** RAISED 2026-08-18 (FS-763): **branch protection is now ENABLED on `main` on both remotes** — `allow_force_pushes: false`, `allow_deletions: false`, `enforce_admins: true`, one required approving review. That is what 3.4.5 (enforce access restrictions associated with change) actually asks for, and it is the line that would have stopped the 2026-08-15 force-push of all 17 branches. STILL PARTIAL, for three specific reasons rather than as a hedge. **Required status checks are not configured**: `main` reports zero check runs, so requiring a context that never arrives would make the branch permanently unmergeable — the 27 jobs gate pull requests, they do not yet gate `main` as a protection rule. **The integration branch is unprotected**, deliberately, because four lanes push to it directly and enabling it without notice stops their work. And **four accounts hold repository admin**, any of which can disable the protection just set, so least-privilege review is outstanding. What changed about the incident: the actor IS now identified — repository events show 14 branches force-pushed in 36 seconds by `SoundSafe-Dev`, mirrored to both remotes, which is machine-driven and consistent with the local automation lead rather than an external attacker. The credential still needs rotating, which is a web-session action no script can perform.
...
**Assessment.** A nightly `pg_dump -Fc` uploads with `--sse AES256` and reads the object back to verify it, from a hardened pod. RAISED 2026-08-20 (FS-811): immutability is no longer "the operator's job" described in a sentence. `base/scripts/bucket-immutability.sh bootstrap` creates a bucket with versioning, **Object Lock in COMPLIANCE mode**, the public-access block, default encryption and a lifecycle rule for retention; `verify` checks a live one read-only. COMPLIANCE rather than GOVERNANCE is deliberate — GOVERNANCE is bypassable by a principal holding `s3:BypassGovernanceRetention`, which is exactly what an attacker who has compromised the account grants themselves. And nobody has to remember to run it: the `backup-immutability-check` CronJob runs `verify` weekly with the credentials the cluster already holds, and `BackupBucketNotImmutable` pages when it fails. The same slice found the CNPG WAL archive pointing at the **in-cluster** SeaweedFS — an archive inside the failure domain it protects, which silently turned the documented 5-minute site-loss RPO into 24 hours. Both environment overlays now patch it off-cluster, asserted by `test_the_wal_archive_leaves_the_cluster.py`. STILL PARTIAL, for narrower reasons than before: `bootstrap` has not been run against the real bucket (creation is a deliberate human act, and the weekly check will report the gap until it is), the off-cluster endpoints are placeholders that fail rather than real values, and **RPO remains up to 24 hours in every environment** because the CNPG cutover has not happened — point-in-time recovery is now proven by a drill (FS-802..806) rather than merely described, which is a different claim from being available.
...
EVIDENCED 2026-08-28, not hypothetical any more. An engineer left, his three subsystem lanes were reassigned in the same change that records this, and the repository contained no runbook, checklist or owner for revoking his access — the gap this note described, met in practice. What the reassignment DID surface is that the technical half above is not the hard part: the deferrals in the test suite carrying his name were found by grepping for it, which is not a process either. Two access facts are open and belong to a human, not to code: his GitHub collaborator/admin rights on both remotes, and whether he held any credential in scope of the 2026-08-15 force-push incident whose rotation is still outstanding. A departure 13 days after that incident is the worst time for this to be undefined.
CLOSED IN PART, 2026-08-31: `docs/runbooks/engineer-offboarding.md` is the missing process, written the day after it was first needed rather than by its due date. It leads with the constraints rather than the commands — two remotes, a 7-day refresh window that outlives deactivating the account, deactivate-never-delete, and the grep that finds register entries which have quietly stopped having an owner. `test_the_offboarding_runbook_is_accurate.py` holds its load-bearing claims so the document cannot age silently. What remains organizational is the trigger: somebody has to notice a departure and open the page, and no code path observes a resignation.
