...
test.describe('data reaches the screen', () => {
  ...
  /**
   * NO LOGIN HERE ANY MORE (FS-452). This file used to authenticate once in a `beforeAll`
   * and replay the state into each test — a fix for the 10/minute rate limit that worked
   * and did not travel: `writes-actually-persist.spec.ts` hit the same ceiling three days
   * later and had to solve it again.
   *
   * The suite now authenticates once in a setup project and every spec inherits the state,
   * so this is a navigation, not a login.
   */
