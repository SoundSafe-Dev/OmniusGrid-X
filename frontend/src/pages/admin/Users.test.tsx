/**
 * The admin Users page — moved here 2026-08-08 with the page itself.
 *
 * `UsersPage` lived in `AdminPages.tsx` and now lives in `Users.tsx`: the merge that
 * brought Hridyansh's invitation surface split it out. These describes followed it rather
 * than being deleted with the import, because what they pin is not specific to whose file
 * the page is in — a failed write must not read as a success, deactivation must be worded
 * as deactivation, and a capped list must say it was capped.
 *
 * Two of them were adapted where his page is legitimately different: it provisions by
 * INVITATION rather than direct creation, so the affordance is "Invite" and the failure
 * path under test is the invite. The property is unchanged.
 */
