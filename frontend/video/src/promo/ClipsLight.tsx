...
/**
 * LIGHT-theme duplicates of the micro-demo clips in Clips.tsx —
 * keep copy/timings in sync. ORIGINAL 9:16 motion graphics built from the brand
...
const Shell: React.FC<{ hook: React.ReactNode; children: React.ReactNode }> = ({
  ...
  <AbsoluteFill style={{ background: '#f7f7f8', fontFamily: theme.fontFamily }}>
    <AbsoluteFill
      style={{
        backgroundImage: `linear-gradient(${'#e6e6ea'} 1px, transparent 1px), linear-gradient(90deg, ${'#e6e6ea'} 1px, transparent 1px)`,
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 30 }}>
          ...
          <span style={{ fontSize: 88, color: '#141414', letterSpacing: -1.5 }}>
            <Wordmark />
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
          <span style={{ fontSize: 34, fontWeight: 500, color: '#5c5c62' }}>by</span>
          <div
            style={{
              background: '#ffffff',
              borderRadius: 999,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 150, color: '#141414', letterSpacing: -3, lineHeight: 1 }}>
          <Wordmark />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginTop: 20 }}>
          <span style={{ fontSize: 36, fontWeight: 500, color: '#5c5c62' }}>by</span>
          <div
            style={{
              background: '#ffffff',
              borderRadius: 999,
...
const Chip: React.FC<{ label: string; tone: 'blue' | 'green'; size?: number }> = ({
  ...
  const t =
    tone === 'green'
      ? { border: 'rgba(22,163,74,0.45)', bg: 'rgba(22,163,74,0.08)', color: GREEN }
      : { border: 'rgba(37,99,235,0.35)', bg: 'rgba(59,130,246,0.08)', color: '#2563eb' };
  return (
    <span
      style={{
...
const GoBadge: React.FC<{ scale: number; size?: number; label?: string }> = ({
  ...
  <span
    style={{
      ...
      fontWeight: 800,
      color: GREEN,
      border: '5px solid rgba(22,163,74,0.5)',
      background: 'rgba(22,163,74,0.10)',
      borderRadius: 24,
      padding: `${size * 0.28}px ${size * 0.65}px`,
      whiteSpace: 'nowrap',
      boxShadow: '0 0 60px rgba(22,163,74,0.18)',
    }}
  >
    {label}
  </span>
...
export const ClipAskLight: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
          ...
          {SOURCES.map(([tag, tone, file], i) => (
            <React.Fragment key={tag}>
              ...
              >
                ...
                <span style={{ fontFamily: MONO, fontSize: 56, fontWeight: 700, color: '#141414' }}>
                  {file}
                </span>
              </div>
              {i < SOURCES.length - 1 && (
                <div
                  style={{
                    height: 44,
                    marginLeft: 150,
                    borderLeft: '5px dashed rgba(37,99,235,0.35)',
...
export const CLIP_ASK_LIGHT_DURATION = 465;
...
export const ClipDetentionLight: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 70 }}>
        ...
        >
          <span style={{ fontSize: 46, fontWeight: 800, letterSpacing: 4, color: '#2563eb' }}>
            YMS
          </span>
          <span style={{ fontFamily: MONO, fontSize: 60, fontWeight: 700, color: '#141414' }}>
            TR-2024-004
          </span>
          <span style={{ fontSize: 50, color: '#5c5c62' }}>Dock 4</span>
          <span style={{ marginLeft: 'auto', fontFamily: MONO, fontSize: 60, fontWeight: 800, color: '#141414' }}>
            {hours}h {mm}m
          </span>
        </div>
        {/* the fee, building */}
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 44, ...fadeUp(frame, 20) }}>
          <span style={{ fontSize: 60, fontWeight: 600, color: '#5c5c62' }}>
            Detention accruing
          </span>
          <span
            style={{
          ...
          {flagged && (
            <div
              style={{
                ...
                flexDirection: 'column',
                gap: 30,
                border: '4px solid rgba(22,163,74,0.45)',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 34 }}>
                ...
                <span style={{ fontSize: 56, fontWeight: 700, color: '#141414' }}>
                  Detention risk — 90 min before billing
                </span>
              </div>
              <span style={{ fontSize: 48, color: 'rgba(20,20,20,0.8)' }}>
...
export const CLIP_DETENTION_LIGHT_DURATION = CLIP2_DUR;
...
export const ClipTherapyLight: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 90, alignItems: 'center' }}>
        ...
        >
          ...
          >
            <span style={{ fontSize: 42, minHeight: 60, color: '#5c5c62', fontFamily: MONO }}>
              {dots(0)}
            </span>
            <Chip label="XLSX" tone="blue" size={72} />
            <span style={{ fontSize: 42, color: '#5c5c62' }}>your ERP</span>
          </div>
          {/* the link, drawn when they meet */}
          <div
            style={{
              width: linkW,
              borderTop: '6px dashed rgba(37,99,235,0.45)',
              margin: '0 30px',
              marginBottom: 60,
            }}
          />
          ...
          >
            ...
            <span style={{ fontSize: 42, color: '#5c5c62' }}>your machines</span>
          </div>
        </div>
...
export const CLIP_THERAPY_LIGHT_DURATION = 345;
