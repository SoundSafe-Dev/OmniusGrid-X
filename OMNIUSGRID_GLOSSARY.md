...
| **Defect-class sweep** | The practice this codebase uses for "code that looks wired and cannot work": fix the instance, then ask whether the same *shape* exists elsewhere and check exhaustively. Twenty-one classes swept so far, each with a mutation-tested guard; **negative results are recorded too**, because "proven clean" and "never checked" look identical afterwards. See `docs/engineering/defect-class-sweeps.md` | Backend |
