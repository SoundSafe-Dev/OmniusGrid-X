...
Last reviewed **2026-08-31**. Terms are defined by what the code does rather than by what the
...
| **Connection budget** | The arithmetic nobody had done: every workload's replica ceiling × its per-process pool, against the database's `max_connections`. Measured at **465 against 100** in staging before FS-839 — the backend HPA allows 20 replicas and KEDA allows 12 + 8 + 6 workers, and SQLAlchemy's default pool is 15 per process. Production was shielded only because the CNPG pooler multiplexes, so the environment that broke was the one nobody load-tests. `tests/k8s/check_connection_budget.py` recomputes it per environment on every build | Backend |
| **Pool sizing (`DB_POOL_SIZE` / `DB_MAX_OVERFLOW`)** | Explicit per-process connection limits: 5 + 5 for the API, 2 + 2 for workers, which run one claim loop rather than serving concurrent requests. Before FS-839 neither was set, so the deployed value was SQLAlchemy's own default — a number chosen by the library, not against this deployment's ceilings | Backend |
| **Bulkhead** | A per-tenant cap on requests IN FLIGHT at once (4 per pod), distinct from a rate limit: the rate limit bounds requests per minute and does nothing about ten simultaneous slow ones. Without it a single tenant issuing eleven expensive queries held every pooled connection in a pod, and every other tenant queued behind it on `DB_POOL_TIMEOUT`. Per-process, because `asyncio.Semaphore` is — across N replicas the real ceiling is N × the cap | Backend |
| **Request deadline** | A 55-second server-side timeout, set just BELOW the ingress's 60s so the server gives up first and the caller receives a real 504 rather than the proxy's opaque one. Before FS-845 nothing told the server the client had gone: the handler kept computing, holding its connection and its bulkhead slot, for a response nobody would read. Bounds time-to-first-byte, not the life of a stream — an SSE response legitimately outlives it | Backend |
| **Circuit breaker (cloud)** | Shared primitive in `core/circuit_breaker.py`: opens after N consecutive failures, refuses without attempting the call, and allows exactly ONE trial after the recovery window. Distinct from the edge's uplink supervisor, which is its own older implementation. Applied where a dependency outage otherwise costs every caller a full connect timeout; deliberately NOT applied to Postgres, where `pool_pre_ping` and `DB_POOL_TIMEOUT` already fail fast and a wrong half-open would keep the app down after the database recovered | Backend |
| **Tenant quota** | Per-organisation limits on assets, active seats, storage bytes and export rows. `0` means unlimited, so the feature ships off rather than refusing real work on deploy day — except `MAX_EXPORT_ROWS`, which ships ON because `0` there would preserve an unbounded in-memory spreadsheet build rather than a working feature. Counted from the rows themselves, never from a maintained counter, which drifts in the direction that lets a tenant exceed | Backend |
| **Per-tenant rate limit** | `RATE_LIMIT_PER_TENANT`, keyed on an `org` claim added to the access token. Before FS-843 every budget keyed on the user id, so an organisation's share of the platform grew with its headcount — 500 users meant 500× the budget of a single-user tenant, which made **the noisiest neighbour structurally the largest customer** | Backend |
| **Ingest pressure** | What the cloud tells edge agents about its own load — `normal` \| `elevated` \| `critical` — carried on the heartbeat ack the agent already polls. Kafka gives a producer no view of consumer lag, so before FS-866 an agent drained its buffer at full rate into a pipeline that had started shedding. Every failure direction resolves to `normal`: a mechanism that fails toward throttling silences a fleet on a typo | Both |
| **Load shedding (cloud)** | The ingestion worker dropping readings by priority tier under pressure, with per-tenant overrides. Correct behaviour and, until FS-860, invisible: the only record was a `debug` log line on clusters deployed at `info`, so a tenant's telemetry was discarded and nothing recorded it. Now counted per tenant and tier, and shedding in tier 1 is CRITICAL rather than high, because that tier is documented as never-shed — a loss there is a policy defect, not overload | Backend |
| **Priority tier vs. priority class** | Two different things that both say "priority", and confusing them costs an incident. A **priority tier** (1–5) is about DATA: which readings are shed or drained first, in the edge buffer and the cloud shedder. A **priority class** (`platform-critical` / `-standard` / `-batch`) is about PODS: which workload the kubelet evicts under node pressure. A tier-1 reading can belong to a workload in `platform-batch` | Both |
| **`ResourceQuota` / `LimitRange`** | Namespace-level ceilings, added in FS-849 because per-container limits cannot bound what the replica CEILINGS add up to. Sized from the manifests (production demands 17.6 CPU / 35.2 Gi at every ceiling) and guarded, because a quota too tight is its own outage: once `requests.cpu` is exhausted the namespace stops admitting pods, so the autoscaler scales up and the replicas sit Pending | Backend |
| **`preStop` (connection draining)** | A five-second pause before SIGTERM, on the workloads a Service routes to. On pod deletion, SIGTERM and endpoint removal happen **in parallel** and removal has to propagate to kube-proxy and the ingress, so a container could shut down while still being routed to. `maxUnavailable: 0` does not help — it counts replicas, it does not drain connections. The symptom was a few 502s on every rolling deploy, thin enough to be filed as a flaky client. Deliberately NOT on the workers, which sit behind no Service | Backend |
| **Startup probe** | A probe that suspends liveness until a slow-starting container is ready. Its absence on `timescaledb` meant liveness killed the database **60 seconds** into WAL recovery (30 + 3×10), and each kill left more WAL to replay — so the loop diverged rather than converging, and the database every restore runbook depends on could not have started from a dirty shutdown | Backend |
...
| **Retention vs. SLO window** | The relationship that made the contractual number wrong. `slo_rules.yml` computes the error budget as `avg_over_time(...[28d])`; Prometheus kept **15 days**. Nothing errored — `avg_over_time` averages the samples present and ignores the absent ones — so the figure was confident and derived from a fortnight, and because the missing half is always the OLDEST, a bad start to a month vanished from that month's budget. **The error flattered.** Retention is 35d now, the volume grew to hold it (retention the disk cannot keep is not retention — the database evicts the oldest blocks and the window silently shortens again), and the requirement is DERIVED from the rules, so widening a window without widening retention fails the build | Backend |
| **`opsgrid_telemetry_shed_total`** | Readings dropped by cloud load shedding, labelled by tenant and priority tier. Labels are bounded deliberately: `metric_name` is caller-supplied and would let a misconfigured device mint unbounded series, which is the cardinality failure `prometheus_tsdb_head_series` alerts on. The metric name stays in the debug log for anyone investigating one tenant | Backend |
| **`opsgrid_db_pool_connections`** | SQLAlchemy pool state read at SCRAPE time rather than maintained on every checkout — hand-kept counters drift from the pool the first time a connection is invalidated rather than returned, which is exactly when the pool is under stress and the number matters. `overflow()` counts from `-pool_size`, so it is clamped; exporting it raw would read as nonsense across the entire healthy range | Backend |
...
| **Session-scoped GUC leak** | `set_config('app.current_org_id', …, is_local=false)` — a tenant binding scoped to the SESSION rather than the transaction. SQLAlchemy's `reset_on_return="rollback"` clears transaction-local settings and leaves session-level ones standing, so the value rides the pooled connection to the next checkout. Worse than an unbound reader, which sees zero rows and fails closed; inheriting a stale tenant makes the same path a successful cross-tenant read (FS-1017) | Backend |
| **Re-bind after commit** | Calling `_set_org` again after each `commit()` inside a loop. A transaction-local tenant binding dies at the first commit, so a loop that commits per item (deliberately, for crash semantics) issues every later statement unbound — under FORCE RLS that matches nothing: zero rows changed, no exception, success reported. The alternative that "fixes" it by widening scope produces the leak above | Backend |
| **Envelope vs. vendor shape** | The receiver's own `event_type`/`event_id` contract versus what a SaaS vendor actually POSTs. No vendor speaks the envelope, so a webhook that authenticates correctly can still be rejected for lacking fields it never sends — QuickBooks deliveries had never once been accepted (FS-996). `services/erp_webhook_payloads.py` translates | Backend |
| **CloudEvents** | The CNCF event envelope (`specversion`, `type`, `id`, `source`, `data`) Intuit requires for QuickBooks webhook payloads from 2026-05-15. Distinguishable from the legacy `eventNotifications` shape by its top-level fields, so both can be handled without guessing | Backend |
| **Derived event id** | A deterministic id hashed from a vendor's own fields (realm + entity + operation + `lastUpdated`) for a sender that supplies none. The point is dedup: a random id satisfies the receiver and silently defeats its `UNIQUE(source_system, event_id)` constraint, so retries insert duplicates while looking correct | Backend |
| **Savepoint dedup** | Inserting each event of a batched delivery inside its own `begin_nested()`. Dedup runs through a UNIQUE constraint rather than check-then-insert, so with several events in one transaction a single `IntegrityError` would roll back the ones that were new — a batch overlapping one already-seen change would discard all the others | Backend |
| **GetFeed** | Geotab's supported high-volume extraction method: polling with a continuation token (`fromVersion`/`toVersion`), capped at 50,000 records per call with ≥1010ms recommended between calls. Geotab has no signed webhook — its "Web Request" rule POSTs form-encoded data with no HMAC and no mTLS — so a feed, not a push, is the integration shape | Backend |
| **Shard redirect** | MyGeotab's `Authenticate` answering with a `path` naming a different server, because databases are sharded. Authenticating at `my.geotab.com` for a database living elsewhere SUCCEEDS and returns the real host; ignoring it sends every subsequent call to a server without the data | Backend |
| **Service Account (Geotab)** | The credential type Geotab requires for API users, replacing a personal username/password login. A client built against `GEOTAB_DATABASE/USERNAME/PASSWORD` targets the shape being retired | Backend |
| **Unbounded session** | `aiohttp.ClientSession()` built with no `ClientTimeout`. aiohttp's default is no total timeout, so a host that accepts the connection and stops responding hangs the coroutine forever — holding a pool slot, never reaching the retry classifier, never counted by the circuit breaker. A failure that has not happened yet (FS-1008) | Backend |
| **`sock_read` vs `total`** | Two different bounds for two different calls. A metadata GET is bounded by total elapsed time; a large artifact download must NOT be, or a healthy transfer is aborted for being large — it is bounded by time-between-bytes instead, which still catches a stalled socket | Backend |
| **Vendor hypothesis** | A finding derived from a vendor's deprecation notice rather than from this tree. It states that the world changed, not that the change reached us — 8 of 9 in Wave A closed on measurement. Distinct from a grep finding, which is an observation (rule 300) | Both |
...
| **SLI (Service Level Indicator)** | The measurement an SLO is judged against. Here: `probe_success × (1 − 5xx_ratio)` — reachable, measured from outside the process, times correctly-served, measured from inside. Both factors are necessary: an SLI built on the second alone cannot see a total outage, because a dead backend exports no series at all rather than a series reading zero | Backend |
| **SLO (Service Level Objective)** | The target the SLI must hold: **99.9% over a rolling 28-day window**. The window is contractual — before FS-772 the longest recording rule was 6h, so nothing in the repository could answer "what was availability this month" | Backend |
| **Error budget** | The 0.1% an SLO permits — **40 minutes 19 seconds** per 28 days. `job:slo_error_budget_remaining:ratio28d` reports it as 1.0 untouched / 0.0 exhausted / **negative = the window is already missed**, and `:minutes28d` in the unit an incident review argues in | Backend |
| **Burn rate** | How fast the error budget is being spent, relative to spending it evenly. Alerted multi-window: 14.4× over 1h **and** 5m pages; 6× over 6h **and** 30m tickets. Two windows because a single short one is noisy and a single long one is slow | Backend |
| **Blackbox probe** | A synthetic request issued by an exporter running **in a different process from the thing it measures**. That separation is the entire point: `probe_success` still reports — reporting 0 — when the backend is gone, which is exactly when a self-reported metric ceases to exist | Backend |
| **`probe_success`** | The blackbox exporter's per-target verdict, 1 or 0. The primary input to availability. Its ABSENCE is a distinct state from 0 and pages via `ProbeSignalMissing`: "I cannot measure availability" must never be read as "availability is fine" | Backend |
| **node-exporter** | Host-level metrics (filesystem, memory, load) as a DaemonSet plus a compose service. `DiskSpaceCritical` (CRITICAL) and `HighMemoryUsage` (HIGH) read `node_*` and it existed in **no environment**, so both were inert everywhere for their whole lives | Backend |
| **postgres-exporter** | Database metrics, with the queries declared in-repo (`infra/prometheus/postgres_queries.yaml`) rather than relying on an upstream default collector set. `SlowDatabaseQueries` named `postgresql_stat_activity_max_tx_duration` — a series no exporter has ever produced — so it would have stayed inert through the very deployment meant to fix it | Backend |
| **`absent()` alert** | A rule that fires when a series is MISSING. The only construct that distinguishes silence from health: a Prometheus that stopped scraping, an exporter that died, and a perfectly well system all produce the same thing — no firing alerts | Backend |
| **Dead man's switch** | `DeadMansSwitch` fires permanently and on purpose, routed to a `watchdog` receiver on infrastructure that does not share a failure domain. **Its value is entirely negative** — an external watcher alerts when it STOPS arriving. Every other rule goes quiet when the monitoring stack dies | Backend |
| **Inert alert / green-by-vacancy** | A rule that lints, deploys and can never fire, because the series it watches is produced by nothing. Indistinguishable from a healthy system, and its promtool test still passes — promtool hand-writes its input series, proving the rule fires GIVEN the series, never that anything produces it. FS-774 found nine, three of them `critical` | Backend |
| **Flag-gated metric** | A metric that is DECLARED but never observed, because its only write site sits behind a feature flag that is off. `prometheus_client` omits a metric with no children from `/metrics` entirely, so an AST sweep over constructor calls calls it "exported" while `APIHighErrorRate` (CRITICAL) could not fire. Guarded by `test_no_alert_rides_on_a_disabled_feature_flag.py` | Backend |
| **Block-scalar blind spot** | A sweep that reads YAML with a per-line regex captures `expr: |` as the literal string `"|"` and never sees the expression beneath. A quarter of `alerts.yml` was outside the sweep that checks alerts watch real series. See rule 272 | Backend |
| **Tail sampling** | Deciding which traces to keep once the trace is COMPLETE, so the decision can be made on whether it turned out to be interesting — errors, latency over 2s, the ingestion path, always kept; 5% of the rest for a baseline. Head sampling must decide at the first span, before the error exists. Ships only in the collector's `contrib` distribution | Backend |
| **Badger (Jaeger storage)** | Jaeger's embedded key-value store, on a PVC. `all-in-one` defaults to memory, so every trace was lost on restart — and a restart is what an incident produces, which made tracing useful only for live debugging, at the moment nobody has time to look. `BADGER_EPHEMERAL=false` is the part that actually makes it persistent | Backend |
| **promtail** | Loki's log shipper, a DaemonSet because pod logs live on the node's filesystem. Existed for docker-compose only, so every runbook step reading "check the container logs" — including the one `WorkerCrashLooping` links to — was unexecutable in production | Backend |
| **Inhibition** | Suppressing an alert that a firing one already explains. Must key on the SUBJECT two alerts share (the volume, the agent, the instance), not on `alertname`: the original rules required one alertname to hold two severities, which no rule does, so both were structurally incapable of matching | Backend |
| **`*_file` secret ref** | Alertmanager reads secrets from mounted FILES; it does not expand `${ENV}`. And an unexpanded placeholder is worse than a wrong value — `${SLACK_WEBHOOK_URL}` is not a URL, so the config is invalid and **Alertmanager exits**, having never delivered an alert | Backend |
| **Parsing is not loading** | A test that YAML-parses a config answers a question the consuming binary need not agree with. `test_alert_routing_coverage` verified every severity had a route in a file Alertmanager refuses to start on. Where a real consumer can render a verdict — `amtool check-config`, `promtool test rules`, a browser loading the built bundle — run it. See rule 273 | Both |
| **Uptime commitment** | `docs/engineering/uptime-commitment.md` — what is measured, from where, over what window, with what exclusions, plus a section stating which of its numbers the system does not yet meet. Deploys, migrations, scaling and certificate expiry are explicitly NOT excluded: an SLA that excuses self-inflicted downtime measures nothing a customer cares about | Both |
| **RED (Rate, Errors, Duration)** | The three questions a request-serving service is judged on, one dashboard. 4xx is plotted beside 5xx deliberately — a 4xx spike with flat 5xx is a client or contract change, and reading only 5xx makes those look like nothing happened | Both |
| **Capacity forecast** | Not fullness but time-to-full (`predict_linear`). A volume at 60% and falling fast needs action before one at 80% and flat, and only the second is visible on a fullness panel | Backend |
| **Base backup** | A full physical copy of the data directory, which WAL replays **on top of**. WAL alone restores nothing — it is a journal of changes to a database that must already exist — so continuous archiving with no base backup is a repository that can recover nothing. The two are a pair, which is why the CNPG cluster ships a `ScheduledBackup` beside its `barmanObjectStore` | Backend |
| **`archive_timeout`** | Forces a WAL segment switch after a set interval even when the segment is not full. **This is the parameter that bounds RPO**, and it was unset: Postgres archives a segment when it FILLS (16 MB), so on a quiet system the tail of the log sat unarchived for hours. WAL archiving existing is not the same as RPO being bounded | Backend |
| **PITR (point-in-time recovery)** | Restoring to a chosen instant by replaying WAL onto a base backup. The property that matters is not "the restore completed" but that data destroyed *after* the target returns and writes made after it do not — a restore that returns *a* database is not one that returns *the* database as it was at 14:32 | Backend |
| **Recovery target** | The instant PITR replays to (`recovery_target_time`). Chosen *before* the mistake — a target after it recovers the mistake faithfully, which the drill's mutation test demonstrates | Backend |
| **Failure domain** | The blast radius of one failure. **A backup that shares a failure domain with its source is not a backup of that domain**: the WAL archive was written to a single-replica object store inside the cluster it protected, so a cluster loss took the archive with it and silently turned a 5-minute RPO into 24 hours. Every signal stayed green, because archiving worked right up to the moment it was needed | Backend |
| **Object Lock** | S3 write-once retention. The control that survives a **compromised credential** rather than a lost datacentre: versioning alone leaves old versions that a key holder can still delete, and Object Lock makes that impossible for the retention period | Backend |
| **COMPLIANCE vs GOVERNANCE mode** | GOVERNANCE can be bypassed by a principal holding `s3:BypassGovernanceRetention` — precisely the permission an attacker who has compromised the account grants themselves. COMPLIANCE cannot be bypassed or shortened by anyone including root, and the trade is real: an object locked by mistake cannot be removed until its retention expires, and you pay to store it | Backend |
| **PgBouncer pooler** | Transaction-mode connection multiplexer in front of the database. The HPA allows 20 backend replicas plus KEDA workers against `max_connections`, and the pooler is what makes that arithmetic survivable. Repointing the app at it was a manual runbook step until the `cnpg-pooler` kustomize component (FS-801) — and a manual step in a runbook is a step that gets skipped | Backend |
| **Cutover preflight** | `tests/k8s/preflight_cnpg_cutover.py` — compares row counts in the tables migrations cannot reconstruct before letting a deploy repoint the app at a new database. An empty-but-healthy cluster accepts the connection, the migration Job builds the schema, the app answers 200, and every customer sees an empty product while the SLI reports perfect health | Backend |
| **Digest pin** | `image:tag@sha256:…`. A tag is repointed by its publisher whenever they cut a release, so a tag is documentation and only the digest is a guarantee. Pinning without a bump process trades floating-and-unreproducible for frozen-and-stale, which is why `check_base_images_are_current.py` runs nightly | Backend |
