...
rules, each written after a defect that a weaker check had missed. Rules 21–303 are recorded in
...
```bash
cd backend && pytest          # 5,650+ pass, ~110 skip. Docker is OPTIONAL: the real-DB
...
uvicorn and drives all 552 documented operations with generated input (~8 min):
...
| **DevOps** | GitHub Actions CI/CD with **28 blocking jobs and 1 advisory** across `quality-gates.yml` and `ci-cd.yml`, counted by `test_ci_gate_count_is_accurate.py` so this number cannot go stale (tsc/eslint/vitest/Playwright, the full backend suite against a real TimescaleDB, migration-chain hygiene, an API contract ratchet over all 552 documented operations, a k6 smoke load test against a real running app, supply-chain: pip-audit/npm-audit/Trivy, and the Kubernetes gates: manifest validation, NetworkPolicy simulation, kind smoke test, Calico policy-enforcement test, plus three capacity checks added in Wave 3 — the **connection budget** (every replica ceiling x its pool against `max_connections`, following the pooler where one is in the path), the **namespace quota** against what those same ceilings demand, and **disruption readiness** (preStop where a Service routes to it, spread constraints, and a priority class that exists); plus the RAG suite, which runs as a reusable workflow in `rag-ci.yml`), kustomize deploys with operator-gated platform stacks, Kubernetes base incl. workers + Redis + db-migrate Job, checksum-tracked SQL migration runner |
...
| **Cache / job store** | Redis — rate limiting, cross-worker idempotency, async export job store, and the ingest-backpressure signal (FS-866). It previously appeared only as a NetworkPolicy destination with no Service behind it, so the always-on auth limiter 500'd every login when it was unreachable. **It is not purely a cache**, which is why it stays a singleton by decision rather than by omission: four consumers degrade safely, but `bulk_processor` and `export_processor` keep job state there and nothing else holds it — so an in-memory fallback would be worse than none (a poll landing on a pod that never saw the job reports it does not exist). The failure is at least legible now, 503 rather than 500, and moving that state to Postgres is the recorded fix. One pool per process since FS-847; there were seven | [`base/redis-statefulset.yaml`](infrastructure/k8s/base/redis-statefulset.yaml) |
...
| **CI safety** | **28 blocking jobs and 1 advisory** on every branch push — the same figure the **DevOps** row of the capability table states, asserted by `test_ci_gate_count_is_accurate.py`. It read **14** here and **31** there until FS-797, when the guard was found counting the `on:` triggers `pull_request:` and `push:` as jobs. Backend: `backend-realdb` (schema parity, tenant isolation + RLS, timestamp defaults — against an ephemeral TimescaleDB, because RLS and server defaults are both no-ops on SQLite), `backend-full` (**5,650+ tests** — the whole suite bar the Kafka e2e, which runs in its own job; the figure is a FLOOR asserted by `test_readme_test_count_is_not_stale.py`, because the exact number was written down once as 2,149 and was a thousand short within weeks), `backend-kafka-e2e` (container e2e in its own process), `migration-hygiene` (duplicate prefixes; and since FS-578 the suite also applies the whole chain to an empty database and **re-runs every migration at its own point in it** — the runner executes statements one at a time in autocommit, because continuous aggregates refuse a transaction block, so a file that fails halfway has committed its earlier statements and recorded nothing, and running it again is the only recovery there is. 22 files look non-idempotent to a text search; **4 are**, and none of them can be repaired — editing an applied migration is checksum drift). Kubernetes: `k8s-manifests` (build + kubeconform + placeholder-credential check, **per environment** — the stacks used to be validated one way and applied another, which is how staging never had monitoring applied at all; plus a namespace/scale-target lint, a replica-floor check against each autoscaler's declared minimum, a secret-source pairing over BOTH provisioning paths, and a check that the canonical README names every buildable tree), `netpol-simulate`, `k8s-smoke` (kind: real operator webhooks), `k8s-netpol` (kind + **Calico**: policies genuinely enforced, 19 allow/deny cases), `netpol-coverage` (every workload in a default-deny namespace has a policy in both directions — the gap that killed tracing), and the three capacity gates from Wave 3: `check_connection_budget.py` (staging demanded **465 connections against a limit of 100** before FS-839 — production was shielded only by the CNPG pooler, so the environment that broke was the one nobody load-tests), `check_resource_quota_fits.py` (a quota too tight is its own outage: once `requests.cpu` is exhausted the namespace stops admitting pods, so the autoscaler scales up and the replicas sit Pending), and `check_disruption_readiness.py` (which also refuses a cluster-scoped `PriorityClass` inside the namePrefixed base, where three tiers rendered as nine objects and a staging pod could outrank a production one). Plus `prometheus-rules` (lints `alerts.yml` + `slo_rules.yml`, checks **both** Prometheus configs, and runs the alert unit tests — **globbed, not listed**, and **all 79 alert rules are now provably FIRABLE** rather than merely well-formed — each driven true from a series the product publishes, each with a must-stay-quiet companion, and the `UNTESTED` set in `test_every_alert_rule_is_provably_firable.py` went 23 → 15 → **0** and is closed — though FS-774 showed that firable is not the same as connected: a quarter of `alerts.yml` is written as YAML block scalars, and the sweep checking that each rule's metrics exist read `expr:` one line at a time, so it captured `"|"` and never examined them: `check rules` cannot tell a rule that fires from one that never can, which is how `EdgeAgentBufferHigh` stayed unfirable for its whole existence: they were six filenames written out, so a new one ran only if somebody remembered to edit the workflow, and an alert test that does not run is indistinguishable from one that passes), `frontend-e2e-authenticated` (stands up Postgres + migrations + demo data + uvicorn and asserts the dashboard shows **non-zero** data — an element-visibility check would have passed against the FS-191 tenancy bug), `supply-chain`, `repo-hygiene`, frontend unit + e2e. The FS-969→1218 sprint added five zero-tolerance guards, each closing a class whose failure mode is silence rather than an exception: `test_nothing_swallows_baseexception.py` (a bare `except:` catches `asyncio.CancelledError`, so a shutdown mid-loop is absorbed and the process appears to **hang** rather than stop), `test_every_outbound_http_call_has_a_timeout.py` (bare `aiohttp.ClientSession()` has no total timeout — 46 call sites, of which a `ClientTimeout` grep reported ten as compliant because they set one *somewhere*), `test_the_tenant_guc_is_transaction_scoped.py` (a session-scoped `set_config` rides the pooled connection to the next checkout, turning a fail-closed unbound read into a cross-tenant one), `test_every_credential_endpoint_is_rate_limited.py` (which found an **unauthenticated** SSO callback that provisions user rows), and `test_the_grafana_datasources_could_connect.py` (a datasource naming a database, a role and a password that between them had never existed). The edge agent's **119 DDIL scenarios** run nightly rather than per-PR (`pytest -m ddil`) because a 400,000-row drain measurement is not a per-PR concern — but the cross-repo parity guards that stop the agent and backend disagreeing about priority tiers or wire codecs are unmarked and gate every push | `.github/workflows/quality-gates.yml` |
...
- **FIPS-capable images, and a runtime check that they are actually enforcing**: all three application images are UBI9 (Debian and musl have no FIPS-validated OpenSSL), and because a container inherits the host kernel's FIPS state — a capable image with FIPS off is indistinguishable from an enforcing one — `REQUIRE_FIPS_MODE` probes at startup whether the process actually refuses unapproved cryptography, and refuses to serve when it cannot prove it
- **Readings that say what their clock was worth**: the agent's clock-skew offset was computed for years and never applied to a telemetry timestamp — `correct()` had no callers, while the module docstring said it did. Timestamps are now corrected at send with the raw edge clock preserved beside them, and every row carries `time_quality` (synced / holdover / unsynced / unknown) so an air-gapped device's data says it cannot be trusted for ordering instead of looking identical to a synchronised one
...
- **Rate Limiting**: 100 req/min per user, **2,000 req/min per tenant** and 1,000 req/min global, using slowapi and Redis. The per-tenant dimension arrived with FS-843: before it every budget keyed on the user id, so an organisation's share of the platform grew with its headcount — 500 users meant 500x the budget of a single-user tenant, which made the noisiest neighbour structurally the largest customer, and the only containment lever was throttling one user at a time. Alongside it: **per-tenant concurrency** (4 in-flight requests per pod, so one tenant cannot take every pooled connection), **volume quotas** on assets, seats, storage and export rows, and a **55-second request deadline** just under the ingress timeout — before which the server kept computing for callers the ingress had already hung up on, holding a connection and a bulkhead slot for a response nobody would read
