...
async def receive_erp_webhook(
    ...
    x_integration_id: Optional[str] = Header(default=None),
    x_organization_id: Optional[str] = Header(default=None),
):
    """Authenticate, dedupe, and store an inbound ERP webhook event."""
    # These identifiers are routing hints, not authorization. They select one
    # candidate tenant/integration; the HMAC must still prove possession of
    # that integration's secret before any event is written.
    try:
        organization_id = UUID(str(x_organization_id))
        integration_id = UUID(str(x_integration_id))
    ...
    async with tenant_session(organization_id) as db:
        ...
        secret = (
            (integration.configuration or {}).get("webhook_secret")
            if integration is not None
            else None
        )
        if integration is None or not verify_signature(
            secret,
            event_data,
            x_webhook_signature,
        ):
            # The response stays generic: routing hints must not become an
            # integration/tenant existence oracle.
            logger.warning(
        ...
        # event_id) constraint, not a check-then-insert: two concurrent
        # deliveries race safely at INSERT and the conflict is a duplicate.
