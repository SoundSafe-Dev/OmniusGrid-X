"""Epicor correlation patterns (FS-55x).

WHY THIS FILE EXISTS. Epicor has a working connector, stores raw records, and
...
The analyzers below read what `ERPDataTransformer.transform_epicor_*` produces, field
...
class EpicorCorrelationPatterns:
    ...
    def __init__(self, organization_id: str, integration_id: str):
        ...
        logger.info(
            "epicor_correlation_patterns_initialized",
    ...
    def _result(
        ...
        return {
            ...
            "entity_id": entity_id,
            "domain": self.EPICOR_DOMAIN_MAPPINGS.get(entity_type),
            "anomalies": anomalies,
        ...
        result = await db.execute(
            ...
            .where(
                and_(
                    ...
                    ERPEntity.source_system == "Epicor",
                )
            )
            .limit(500)
        )
