"""NetSuite correlation patterns (FS-557).

WHY THIS FILE EXISTS. NetSuite has a working connector, it stores raw records, and
`erp_sync_correlation.route_for()` returned `None` for every one of them — so every NetSuite
sync completed, wrote its rows, and reported `skipped: unrouted`. The customer saw a
successful integration and an empty correlation list, with nothing anywhere saying the vendor
was not analysed.

WHY NOT REUSE ORACLE'S. The route registry states the rule in its own header: *"A vendor with
no verified pair stays out. Reusing another vendor's transformer would yield empty normalized
records and a confident report of zero anomalies."* That is the outcome to avoid — not an
error, but a clean bill of health computed from a record of `None`s.

The analyzers below read the normalized shape `ERPDataTransformer.transform_netsuite_*`
produces, field for field. Two are NetSuite-specific rather than shared:

  * **Overdue invoices** must compare against a mapped status. NetSuite says "Paid In Full",
    not "paid", so a shared comparison would report every settled invoice as overdue —
    which is why the mapping lives in the transformer and is asserted by its test.
  * **Inventory shortfall** uses `quantityAvailable` (on-hand minus committed) against
    `reorderPoint`. Oracle has no reorder point in its normalized shape, so this analyzer has
    no counterpart to share.
"""
...
class NetSuiteCorrelationPatterns:
    ...
    def __init__(self, organization_id: str, integration_id: str):
        ...
        logger.info(
            "netsuite_correlation_patterns_initialized",
    ...
    def _result(
        ...
        return {
            ...
            "entity_id": entity_id,
            "domain": self.NETSUITE_DOMAIN_MAPPINGS.get(entity_type),
            "anomalies": anomalies,
        ...
        result = await db.execute(
            ...
            .where(
                and_(
                    ...
                    ERPEntity.source_system == "NetSuite",
                )
            )
            .limit(500)
        )
