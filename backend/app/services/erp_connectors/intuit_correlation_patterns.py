"""Intuit correlation patterns (FS-55x).

WHY THIS FILE EXISTS. Intuit has a working connector, stores raw records, and
...
The analyzers below read what `ERPDataTransformer.transform_intuit_*` produces, field
...
class IntuitCorrelationPatterns:
    ...
    def __init__(self, organization_id: str, integration_id: str):
        ...
        logger.info(
            "intuit_correlation_patterns_initialized",
    ...
    def _result(
        ...
        return {
            ...
            "entity_id": entity_id,
            "domain": self.INTUIT_DOMAIN_MAPPINGS.get(entity_type),
            "anomalies": anomalies,
        ...
        result = await db.execute(
            ...
            .where(
                and_(
                    ...
                    ERPEntity.source_system == "Intuit",
                )
            )
            .limit(500)
        )
