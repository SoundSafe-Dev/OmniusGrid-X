...
class TestTenantIsolationStillHolds:
    ...
    async def test_another_org_cannot_read_the_asset(self, client_b, owned_asset):
        response = await client_b.get(f"/api/v1/oee/current/{owned_asset}")
        assert response.status_code == 404, (
            "org B can read org A's asset OEE; binding the GUC must scope the read, "
            "not open it"
        )
