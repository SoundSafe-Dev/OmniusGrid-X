...
UNHANDLED_BY_AGENT = {
    "model_update": (
        "ModelUpdateExecutor exists and is fully written "
        "(edge-agent/opsgrid_agent/ota/model_executor.py) but is never instantiated in "
        "main.py, so `register()` is never called. Wiring it is ~3 lines in the OTA lane; "
        "it also has zero tests (FS-507), and switching an untested handler into the live "
        "command path is a separate act from noticing it is missing. Owner: Hridyansh."
    ),
}
